  ...
  return (
    <div className="blog-list">
      {blogPosts.map((post, index) => (
        post.private ?
          <PrivateBlogPost key={index} title={post.title} author={post.author} date={post.date} /> :
          <PublicBlogPost key={index} title={post.title} author={post.author} date={post.date} content={post.content} />
      ))}
    </div>
  );
}
